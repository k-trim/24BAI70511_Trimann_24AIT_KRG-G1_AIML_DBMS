...
SELECT * FROM Tbl_Supply_logs;-- DROP TABLES (optional, to avoid errors if re-running)
DROP TABLE IF EXISTS Tbl_Supply_logs;
DROP TABLE IF EXISTS Tbl_Orders;
DROP TABLE IF EXISTS Tbl_Suppliers;
DROP TABLE IF EXISTS Tbl_Products;
...
CREATE OR REPLACE PROCEDURE swap_city(sup_id1 INT, sup_id2 INT)
AS $$
DECLARE
    city1 VARCHAR(100);
    city2 VARCHAR(100);
BEGIN
    SELECT city INTO city1 FROM Tbl_Suppliers WHERE sup_id = sup_id1;
    SELECT city INTO city2 FROM Tbl_Suppliers WHERE sup_id = sup_id2;

    UPDATE Tbl_Suppliers SET city = city2 WHERE sup_id = sup_id1;
    UPDATE Tbl_Suppliers SET city = city1 WHERE sup_id = sup_id2;
END;
$$ LANGUAGE plpgsql;
